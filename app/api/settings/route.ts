...
export async function GET() {
  try {
    const s = await getRawDb()
      .prepare("SELECT bank_name AS bankName,account_number AS accountNumber,account_name AS accountName,qr_object_key AS qrKey,thank_you_text AS thankYouText FROM site_settings WHERE id=1")
      .first<Record<string, unknown>>();
    return NextResponse.json(
      s
        ? { ...s, qrUrl: s.qrKey ? "/api/settings/qr" : null }
        : { bankName: "", accountNumber: "", accountName: "", thankYouText: "Cảm ơn bạn đã đồng hành cùng Hugo Cyberx!", qrUrl: null },
    );
  } catch {
    return NextResponse.json({ qrUrl: null });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  const qr = f.get("qr");
  const old = await getRawDb()
    .prepare("SELECT qr_object_key FROM site_settings WHERE id=1")
  ...
  await getRawDb()
    ...
    .bind(
      String(f.get("bankName") || ""),
      String(f.get("accountNumber") || ""),
      String(f.get("accountName") || ""),
      key,
      String(f.get("thankYouText") || ""),
    )
    .run();
